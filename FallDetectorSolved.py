...
def main_train(model):
    ...
    x_size = len(train_X[0]) #30
    h_size = 50
    y_size = len(train_y[0]) #2
    ...
    b_1 = tf.random_normal([h_size], stddev=0.1)
    b_2 = tf.random_normal([y_size], stddev=0.1)
    ...
    yhat = tf.nn.softmax(tf.matmul(h, w_2) + b_2, name="Yhat")
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(len(train_X)):
            ...
            sess.run([updates, mean_squared_error], feed_dict={X: np.asarray([train_X[i]]), y: np.asarray([train_y[i]])})
...
def main_predict(model, sample):
    ...
    with tf.Session() as sess:
        ...
        for i in range(len(x_data)):
            ...
            y_pred = sess.run(Y_pred, feed_dict={X: input, Y: np.asarray([[0,0]])})
